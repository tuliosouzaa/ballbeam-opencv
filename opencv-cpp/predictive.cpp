...
float erro;
float thenn;
float y, y_ant, yder, ypos, ypos_a;
float pred_integrl;
float pred_derivada;
float pred_control_output;
float pred_erro_a;
float confianca;
float pred_control_output_a;
	...
	for(;;)
	{
		...
		if( !paused )
		{
			...
			if( trackObject )
			{
				...
				if(abs(erro)<=1.5) ellipse( image, trackBox, Scalar(0,255,0), 3, CV_AA );
				else if (abs(erro)>1.5) ellipse( image, trackBox, Scalar(0,0,255), 3, CV_AA );
				...
				if (position != 0)
				{
					...
					if(abs(erro)>1.5) integrl = integrl + erro*(0.3);
					derivada = (erro-erro_anterior)/(0.3);
					control_output = (KP*erro + KD*derivada + KI*integrl);
					erro_anterior = erro;

					//predicao da posicao
					y = position;
					yder = (y - y_ant)/0.3; // deriva
					printf("yder %f \n",yder);
					ypos = y + yder;
					printf("PREDICAO DE POSICAO %f \n",ypos);
					printf("ERRO DE PREDICAO DE POSICAO %f \n",abs(y-ypos_a));
					
					confianca = 1 - abs(y-ypos_a)/460;;
					printf("CONFIANCA %f \n",confianca);
					
					float pred_erro = MV - ypos;
					pred_erro = pred_erro/10;
					if(abs(erro)>1.5) pred_integrl = pred_integrl + pred_erro*(0.3);
					pred_derivada = (pred_erro - pred_erro_a)/0.3;
					pred_control_output = KP*pred_erro + KD*pred_derivada + KI*pred_integrl;
					pred_erro_a = pred_erro;

					if (pred_control_output > 30) 
						pred_control_output = 30;
					else if (pred_control_output < -30)
						pred_control_output = -30;
					...
					printf("SINAL DE CONTROLE PREDICAO %f \n",pred_control_output);
					serialWrite( ((int)control_output*(1-confianca) + pred_control_output*confianca) );
					////////////////////////////////////////////////////////////
					////////////////////////////////////////////////////////////
					y_ant = y;
					ypos_a = ypos;
		...
		clock_t then = clock();
		thenn = thenn + (then-now);

		if (position !=0 && thenn<200)
		{
			Sleep(300-thenn);
		}
		if(position != 0)
